...
The probability of an outcome $Y=1$ given independent variable $X=x$ is mathematically represented by:

$$
p(x) = P(Y = 1 \mid {X = x})
$$
In this case, $x$ is a vector containing our independent variable data (VDP's for each subject). Logistic regression takes the log of the probability $p(x)$ over the anti-probability $1-p(x)$, and fits a linear regression model to $x$: 
